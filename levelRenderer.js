class LevelRenderer {
    ...
    getHuedPlayerSprite(hue, hue2) {
        const topHue = hue || 0;
        const bottomHue = (typeof hue2 === 'number') ? hue2 : topHue;
    ...
    // The player sprite has two base colors baked in: a top region (greenish,
    // base hue ~140deg) and a bottom region (yellowish, base hue ~60deg). Each
    // pixel is routed to the matching hue shift based on its original color so
    ...
    renderPlayer(playerPos, camera, hue = 0, hue2 = 0, name = "", color = "#ffffff", status = null, alpha = 1) {
        ...
        const sprite = this.getHuedPlayerSprite(hue, hue2);
