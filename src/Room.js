...
class Room {
    ...
    roomStatePayload() {
        return {
            ...
            seats: [...this.seats.values()].map(s => ({
                ...
                isBot: s.isBot,
                hue: s.hue,
                hue2: typeof s.hue2 === 'number' ? s.hue2 : s.hue
            }))
        };
    }
    ...
    sendJoinCatchUp(seat) {
        ...
        if (this.levelCode) {
            ...
            if (this.map && this.phase !== PHASE.BUILD) {
            ...
            case PHASE.BUILD: {
                ...
                this.send(seat, {
                    ...
                    payload: {
                        ...
                        pieceId: seat.piece,
                        buildPlaced: seat.buildPlaced,
                        col: seat.buildCursor.col,
                        row: seat.buildCursor.row,
                        rotation: seat.buildRotation,
    ...
    handleSetColorRequest(seat, payload = {}) {
        ...
        let hue2 = Math.round(Number(payload.hue2));
        if (!Number.isFinite(hue2) || hue2 < HUE_MIN || hue2 > HUE_MAX) hue2 = hue;
        ...
        if (this.phase === PHASE.LOBBY) {
            ...
            this.broadcast({
                type: 'COLOR_UPDATED',
                phase: this.phase,
                payload: { seatIndex: seat.seatIndex, hue, hue2 }
            });
        }
    }
    ...
    pickPartySlots(count, allowBomb, guaranteeBomb) {
        ...
        if (guaranteeBomb && !bombDisabled && count > 0 && !slots.some(s => s.pieceId === 'bomb')) {
    ...
    handlePositionSnapshot(seat, payload) {
        ...
        this.broadcast({
            type: 'POSITION_SYNC',
            phase: this.phase,
            payload: [seat.seatIndex, ...payload]
        }, seat.seatIndex);
    }
    ...
    handleReconnect(seat, ws, displayName) {
        clearTimeout(seat.disconnectCleanupTimer);
        seat.ws = ws;
        seat.ip = ws.clientIp || seat.ip || null;
        seat.connected = true;
        seat.isBot = false;
